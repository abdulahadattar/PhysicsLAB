...
function normalizeScientificNotation(input: string): string | null {
  ...
  // This is a simplified normalization. True normalization is more complex for perfect sig fig handling.
  if (coefficient !== 0) {
  ...
  // Using toPrecision can be tricky for exact sig fig matching from string input,
  // so we'll format it simply and compare the numerical values primarily.
  // For display, it's fine, but for comparison, a more robust parser is needed for varied user inputs.
  // Let's try a simpler representation for comparison:
  const coeffStr = String(coefficient);
  return `${coeffStr}e${exponent >= 0 ? '' : ''}${exponent}`; // Remove explicit '+' for positive exponent for broader match
}
...
export default function SigFigsScientificNotationG9Page() {
  ...
  const handleCheckAnswer = () => {
    ...
    if (currentProblem.type === 'count_sig_figs') {
      ...
      const userNorm = normalizeScientificNotation(userAnswer);
      const correctNorm = normalizeScientificNotation(String(currentProblem.answer));
      
      if (userNorm && correctNorm) {
        // Compare coefficient and exponent parts
        const [userCoeffStr, userExpStr] = userNorm.split('e');
        const [corrCoeffStr, corrExpStr] = correctNorm.split('e');

        const userCoeff = parseFloat(userCoeffStr);
        const userExp = parseInt(userExpStr, 10);
        const corrCoeff = parseFloat(corrCoeffStr);
        const corrExp = parseInt(corrExpStr, 10);
        
        // Allow for slight floating point differences in coefficient and ensure exponents match
        // A better approach would be to ensure the user's coeff has the right number of sig figs
        // based on the original value, but that's more complex for this context.
        isCorrect = Math.abs(userCoeff - corrCoeff) < 0.001 && userExp === corrExp;
      } else {
        isCorrect = false;
      }
      correctAnswerDisplay = String(currentProblem.answer); 
    } else if (currentProblem.type === 'from_scientific_notation') {
      // For 'from_scientific_notation', compare numerical values
      const userAnswerNum = parseFloat(userAnswer.trim());
      const correctAnswerNum = parseFloat(String(currentProblem.answer).trim());
      if (!isNaN(userAnswerNum) && !isNaN(correctAnswerNum)) {
        isCorrect = Math.abs(userAnswerNum - correctAnswerNum) < 1e-9; // Compare as numbers
      } else { // Fallback to string comparison if parsing fails (e.g. non-numeric input)
        isCorrect = userAnswer.trim() === String(currentProblem.answer).trim();
      }
    } else if (currentProblem.type === 'round_sig_figs') {
      // For rounding, direct string comparison is usually expected
      isCorrect = userAnswer.trim() === String(currentProblem.answer).trim();
    }
  ...
  const handleNextProblem = () => {
    setCurrentProblemIndex((prevIndex) => {
      const nextIndex = (prevIndex + 1) % problemPool.length;
      if (nextIndex === 0 && prevIndex === problemPool.length - 1) { // Reshuffle if looped
            ...
            <Popover>
              ...
              <PopoverContent className="w-80 text-sm">
                ...
                <ul className="list-disc pl-4 text-muted-foreground space-y-1 text-xs">
                  ...
                  <li>For "to scientific notation", enter in the format X.YeZ (e.g., 1.23e4 or 1.23e-5). Case of 'e' does not matter.</li>
        ...
        <CardContent className="space-y-6">
          <Card className="bg-secondary/30">
            <CardContent className="p-4 md:p-6 space-y-2">
              ...
              <p className="text-2xl md:text-3xl font-bold text-primary text-center my-3 md:my-4 select-all bg-background p-2 md:p-3 rounded-md break-all">
                {currentProblem.value}
              </p>
              {currentProblem.type === "round_sig_figs" && (
                <p className="text-xs md:text-sm text-center text-muted-foreground">(Round to {currentProblem.sigFigsToRound} significant figures)</p>
              )}
            </CardContent>
          </Card>
          ...
          <div>
            ...
            <Input
              ...
              placeholder={
                ...
                currentProblem.type === 'to_scientific_notation' ? "e.g., 1.23e-4 or 1.23E+4" :
                currentProblem.type === 'from_scientific_notation' ? "e.g., 0.00123 or 12300" :
                "Enter number"
              }
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button onClick={handleNextProblem} variant="outline" className="flex-1" aria-label="Go to the next problem">
